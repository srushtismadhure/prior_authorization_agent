...
# carrier.columns = carrier.columns.str.strip().str.lower().str.replace(" ", "_")
# # Select the relevant columns
# carrier = carrier[["code", "description_of_service", "service_category", "approval_rate"]]
# # ---- Add missing procedures manually ----
# additional_services = [
#     {"code": "78815", "description_of_service": "pet scan whole body", "service_category": "Imaging", "approval_rate": "85%"},
#     {"code": "78816", "description_of_service": "pet scan skull base to mid thigh", "service_category": "Imaging", "approval_rate": "85%"},
#     {"code": "95810", "description_of_service": "sleep study", "service_category": "Sleep Medicine", "approval_rate": "90%"},
...
# extra_df = pd.DataFrame(additional_services)
# carrier = pd.concat([carrier, extra_df], ignore_index=True)
# print("✅ Added missing PET and sleep study descriptions.")

# # Clean up text formatting
# carrier["description_of_service"] = carrier["description_of_service"].astype(str).str.lower()
# print("\n✅ First 10 descriptions in carrier file:\n")
# print(carrier["description_of_service"].head(10).to_string(index=False))


# from rapidfuzz import process, fuzz

# def find_related_services(note):
#     note = note.lower()
#     match = process.extractOne(note, carrier["description_of_service"], scorer=fuzz.partial_ratio)
    
#     if match and match[1] > 80:
#         text, score, idx = match
#         matched_row = carrier.iloc[idx]
#         return {
#             "Matched_Description": matched_row["description_of_service"],
#             "Code": matched_row["code"],
#             "Category": matched_row["service_category"],
#             "Approval_Rate": matched_row["approval_rate"],
#             "Confidence_Score": score  # new column!
#         }
#     return None
...
# notes["Needs_Prior_Auth"] = notes["Matched_Description"].notnull()
# notes.to_csv("matched_notes_output.csv", index=False)
# print("✅ Results saved to matched_notes_output.csv")
# open("matched_notes_output.csv")
...
# sample = notes.head(5)   # just take 5 notes
# sample["Summary"] = sample.apply(summarize, axis=1)

# print(sample[["Clinical_Notes", "Matched_Description", "Needs_Prior_Auth", "Summary"]].to_string(index=False))

# sample.to_csv("test_notes_output.csv", index=False)
# print("✅ Saved small test output to test_notes_output.csv")

# # notes["Summary"] = notes.apply(summarize, axis=1)
# # print("✅ Summaries added.")

# # import pandas as pd, os
# # pd.set_option('display.max_colwidth', None)
# # print(notes[["Clinical_Notes", "Matched_Description", "Needs_Prior_Auth", "Summary"]].head(3).to_string(index=False))

# # notes.to_csv("notes_with_matches.csv", index=False)
# # print("✅ Results saved to notes_with_matches.csv")
# # os.system("open notes_with_matches.csv")
